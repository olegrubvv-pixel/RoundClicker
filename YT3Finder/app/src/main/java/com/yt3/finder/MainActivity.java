...
public class MainActivity extends Activity {
    ...
    private TextView stats,status,countText,manualResult,filteredText;
    ...
    @Override public void onCreate(Bundle b){
        ...
        root.addView(text("Сначала удаляет всё, что уже находится в YouTube, потом проверяет остаток",14,Color.rgb(145,154,174)));
        ...
        filteredText=text("YouTube сразу отсеял: 0",15,Color.rgb(111,168,255));stat.addView(filteredText);
        ...
        info.addView(text("Новая схема: перед любой тяжёлой проверкой приложение сначала смотрит точный @handle на нескольких страницах YouTube. Если YouTube явно открывает существующий канал, такой ID сразу удаляется из дальнейшего поиска и заносится в чёрный список. Только то, что этот фильтр не подтвердил как занятое, идёт на многоэтапную глубокую проверку. Ошибка сети или непонятный ответ никогда не считается свободным. Финальную возможность назначить handle всё равно подтверждает только сам YouTube.",13,Color.rgb(215,220,230)));
        root.addView(info);
        ...
        copyAll.setOnClickListener(v->copyAll());handler.post(refreshLoop);
    }
    ...
    private void runManualCheck(){
        ...
        if(!validThree(h)){manualResult.setText("Неверный формат. Нужно ровно 3 символа: a-z, 0-9; _ - . · только посередине.");manualResult.setTextColor(Color.rgb(255,100,110));return;}
        manualCheck.setEnabled(false);manualResult.setText("Сначала ищу @"+h+" в YouTube, затем запускаю глубокую проверку…");manualResult.setTextColor(Color.rgb(111,168,255));
        new Thread(()->{
            String result;
            if(HandleChecker.definitelyOccupiedFast(h))result="occupied";
            else{
                HandleChecker.StrictResult a=HandleChecker.strictCheck(h,true);result=a.status;
                if("yellow".equals(result)){try{Thread.sleep(1200);}catch(Exception ignored){}result=HandleChecker.strictCheck(h,true).status;}
            }
            final String r=result;
            runOnUiThread(()->{
                manualCheck.setEnabled(true);
                if("occupied".equals(r)){manualResult.setText("🔴 @"+h+" — ЗАНЯТ: YouTube явно нашёл существующий канал.");manualResult.setTextColor(Color.rgb(255,80,95));}
                else if("yellow".equals(r)){manualResult.setText("🟡 @"+h+" — сверхстрогий кандидат. Канал публично не найден, но назначение подтверждает YouTube.");manualResult.setTextColor(Color.rgb(244,200,74));}
                else{manualResult.setText("⚪ @"+h+" — НЕ УДАЛОСЬ ПОДТВЕРДИТЬ. Ошибка/лимит/неоднозначность; свободным не считаю.");manualResult.setTextColor(Color.rgb(180,190,205));}
            });
        }).start();
    }
    ...
    private void refresh(){
        int index=prefs.getInt("index",0),total=prefs.getInt("total",51840),count=prefs.getInt("candidate_count",0),removed=prefs.getInt("prefilter_removed",0);
        boolean running=prefs.getBoolean("running",false);
        stats.setText(index+" / "+total);countText.setText("Кандидаты: "+count);filteredText.setText("YouTube сразу отсеял: "+removed);
        start.setEnabled(!running);pause.setEnabled(running);copyAll.setEnabled(count>0);
        status.setText(running?"Сначала отсеиваю найденные в YouTube, оставшиеся проверяю глубже…":prefs.getString("error",index>0?"Пауза. Можно продолжить.":"Готово к поиску."));
        loadCandidates();
    }
    ...
    private void loadCandidates(){
        ...
        for(String h:rows){
            ...
            Button cp=btn("Копировать"),yt=btn("YouTube"),del=btn("Занят ✕");
    ...
    private void deleteCandidate(String h){
        String key=h.toLowerCase(Locale.ROOT);Set<String> blacklist=new HashSet<>(prefs.getStringSet("blacklist",Collections.emptySet()));blacklist.add(key);
